...
#     """
#     assert hasattr(env, "num_action")
#     assert hasattr(env, "reset") and hasattr(env, "step")
    
#     input_arg = len(env.reset()[0])
...
def dqn_learing(
    ...
    # Retrieve an initial observation from the environment
    initial_obs = env.reset()

    # Flatten the observation to ensure it is 1D
    flattened_obs = np.array(initial_obs).flatten()

    # Assign the size of the processed observation to input_arg
    input_arg = len(flattened_obs)
